      ...
      describe("to deployId=" + id, function() {
        it("should deploy without error", function(done) {
          ...
          exec("./src/deployer", function(error, stdout, stderr) {
            ...
            expect(stdout).to.contain(
              clusterName +
                " - Strategy rolling-update cleanup attempting to delete 2 deployments that match the nginx1-deployment deployment group label name=nginx1-pod,strategy!=rolling-update"
            );
            expect(stdout).to.contain(
              clusterName +
            ...
            expect(stdout).to.contain(
              clusterName +
                " - Strategy rolling-update cleanup deleted deployment nginx1-deployment-dep-3"
            );
            expect(stdout).to.contain(
              clusterName +
                " - Strategy rolling-update cleanup successfully deleted 2 deployments"
            );
