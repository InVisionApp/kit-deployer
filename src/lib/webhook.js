...
class Webhook extends EventEmitter {
      ...
      promises.push(
        request({
          ...
          .then(res => {
            this.emit(
              "info",
              "Successfully sent payload to " + phaseStatusMessage
            );
            return res;
          })
          .catch(err => {
            ...
            const errPrefix =
              "Error sending payload to " + phaseStatusMessage + ": ";
            if (err && err.message) {
  ...
  getManifestName(status) {
    ...
    if (_.has(status.manifest, ["metadata", "annotations", aliasNameKey])) {
      ...
      _.has(status.manifest, ["metadata", "annotations", originalNameKey])
    ) {
