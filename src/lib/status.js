    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.options.keepAlive) {
        keepAlive = new KeepAlive(
          "Still waiting for " + resource + ":" + name + " to be available...",
      ...
      // Setup watcher
      const watcher = this.kubectl.watch(
        resource,
        name,
        this.options.pollingInterval
      );
      ...
      watcher.on("change", res => {
        ...
        switch (resource) {
          case "Deployment":
            ...
            if (
              _.has(res, "metadata", "generation") &&
              res.metadata.generation !== undefined
            ) {
              generation = parseInt(res.metadata.generation);
            }
            var observedGeneration = null;
            if (
              _.has(res, "status", "observedGeneration") &&
              res.status.observedGeneration !== undefined
            ) {
              observedGeneration = parseInt(res.status.observedGeneration);
            }
            var unavailableReplicas = 0;
            if (
              _.has(res, "status", "unavailableReplicas") &&
              res.status.unavailableReplicas !== undefined
            ) {
              unavailableReplicas = parseInt(res.status.unavailableReplicas);
            }
            var availableReplicas = null;
            if (
              _.has(res, "status", "availableReplicas") &&
              res.status.availableReplicas !== undefined
            ) {
              availableReplicas = parseInt(res.status.availableReplicas);
            }
            var replicas = null;
            if (
              _.has(res, "status", "replicas") &&
              res.status.replicas !== undefined
            ) {
          ...
          case "Job":
            ...
            var succeeded = null;
            if (
              _.has(res, "status", "succeeded") &&
              res.status.succeeded !== undefined
            ) {
            ...
            if (succeeded !== null) {
              this.emit(
                "debug",
                resource + ":" + name + " has " + succeeded + "/1 succeeded"
              );
              if (succeeded) {
                stop(this);
              }
          ...
          case "DaemonSet":
            ...
            if (
              _.has(res, "status", "desiredNumberScheduled") &&
              res.status.desiredNumberScheduled !== undefined
            ) {
              desiredNumberScheduled = parseInt(
                res.status.desiredNumberScheduled
              );
            }
            var currentNumberScheduled = null;
            if (
              _.has(res, "status", "currentNumberScheduled") &&
              res.status.currentNumberScheduled !== undefined
            ) {
              currentNumberScheduled = parseInt(
      ...
      this.emit(
        "info",
        "Waiting for " + resource + ":" + name + " to be available..."
      );
      watcher.start();
      ...
      timeoutId = setTimeout(() => {
        ...
        reject(
          new TimeoutError("Timeout waiting for " + resource + ":" + name)
        );
